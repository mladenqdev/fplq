...
- **League movement**: standings rows show each team's official current rank versus `last_rank`, with green up/red down and places moved since the start of the GW. Unchanged and newly ranked entries have neutral labels. The league API already includes both ranks, so this needs no extra per-team request.
