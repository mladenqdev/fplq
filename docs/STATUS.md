...
- `packages/shared` (`@fplq/shared`) — FPL types, our DTOs, and ALL pure domain logic (bonus, auto-subs, live points, free transfers, prices, planner `derivePlan`, FDR, `projection`, formatters). Vitest: **86 shared tests pass**. No runtime deps; consumed as TS source.
...
- **League movement**: standings rows show each team's official current rank versus `last_rank` in a dedicated column immediately before GW points, with green up/red down and places moved since the start of the GW. Unchanged and newly ranked entries have neutral labels. The league API already includes both ranks, so this needs no extra per-team request.
