...
Standings rows show the official `rank` versus `last_rank` (the position entering the current GW): green up and red down with places moved, a neutral dash for unchanged and "new" if `last_rank` is zero. Movement has its own column immediately before GW points; rank remains a single value on the left. Very large ranks are shortened on narrow screens, while the accessible team link announces the full rank. This uses the fields already returned by `/api/league/:id`, including for teams other than the saved entry. Standings rows link to `/league/:id/team/:entryId?page=N`. The league page stores pagination in the URL so the team page can return to the same standings page. The team view reuses `/api/entry/:entryId/live/:gw` for the current released gameweek, with the manager/team name, official points and ranks, chip/hit, pitch, bench and player points breakdown. Viewing a rival never updates the saved entry ID or transfer plan. The view handles loading, unavailable picks with retry, invalid team IDs and the pre-season period before public lineups exist. Future lineups and authenticated team changes are not exposed.
